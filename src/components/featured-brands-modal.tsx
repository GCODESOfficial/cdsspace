...
export function FeaturedWorksModal({
	isOpen,
	onClose,
}: FeaturedWorksModalProps) {
	...
	const handleSelectionChange = (work: any, checked: boolean) => {
		setSelectedWorks((prev) => {
			const alreadySelected = prev.find((w) => w.id === work.id);
			if (checked && !alreadySelected && prev.length < 8) {
				return [...prev, work];
			} else {
...
{allWorks.length === 0 ? (
	...
	<div className="space-y-2 max-h-96 overflow-y-auto pr-2">
		{[...selectedWorks, ...allWorks.filter(w => !selectedWorks.some(s => s.id === w.id))]
			.filter(work =>
				work.title.toLowerCase().includes(searchQuery.toLowerCase())
			)
			.map((work) => {
						...
						{isSelected && (
							<div className="flex items-center gap-2">
								...
								<Button
									...
									disabled={
										isLoading || index === selectedWorks.length - 1
									}
									className="h-8 w-8 p-0"
								>
